    ...
    def __call__(self, voxel):
        ...
        if voxel.pressure() < self.pressure_threshold:
            for cell in voxel.list_of_cells:
                if cell.vitality() > self.cycling_threshold and not cell.necrotic():
...
class CellApoptosis(Process):
    ...
    def __call__(self, voxel):
        for cell in voxel.list_of_cells:
            if cell.vitality() < self.apoptosis_threshold and not cell.necrotic():
...
class UpdateVasculature(Process):
    ...
    def __call__(self, world: World):
        ...
        print("\033[1;31;47mRed text on white background\033[0m")
        n_killed = world.vessels_killed_by_pressure(self.pressure_threshold)
