...
def run_calibration(side = 6, a = -7, b = 1, max_n = 100): #side is the length of the cube, a and b are the parameters of the sigmoid function, max_n is the maximum number of capillaries to be simulated
    ...
    for p in range(8): #loop over different pressures
        ...
        for n_idx, n in enumerate(n_values): #loop over different number of capillaries in the cube
            ...
            if p == 0.3 and n == 30:
                #write csv all o2 values to file
                all_o2_values_ = np.array(all_o2_values)
                np.savetxt('all_o2_values_n_' + str(n) + '_side_' + str(side) + '_pressure_' + str(pressure) + '.txt', all_o2_values_)
...
def run_calibration_single(side = 6, a = -7, b = 1, n = 1, p = 0.0): #side is the length of the cube, a and b are the parameters of the sigmoid function, max_n is the maximum number of capillaries to be simulated
    ...
    for _ in range(n_iter): #number of iterations for each n and pressure (50 is enough to have a good estimate of the mean)
        ...
        if plot3D:
            plt.title('n = ' + str(n) + ', side = ' + str(side) + 'um, pressure = ' + str(p))
            #add colorbar
            m = plt.cm.ScalarMappable(cmap='turbo')
            m.set_array(all_o2_values)
            m.set_clim(0, 1)
            plt.colorbar(m)
            plt.savefig('3D_n_' + str(n) + '_side_' + str(side) + '_pressure_' + str(p) + str(_)+ '.png', dpi = 300)
            plt.close()

    return all_o2_values
